  ...
  return (
    <div className="add-property-layout">
      ...
      <div className="add-property-main-container">
        <Sidebar />
        <div className="add-property-page-content-wrapper">
          <div className={`add-property-card-box ${isSuccess ? 'success-mode' : ''}`} dir="ltr">
            {!isSuccess ? (
              <div className="add-property-page-flow">
                <div className="add-property-page-header-row">
                  <div className="header-left">
                    <h1>{t('landlordHomeComponents.listNewProperty', { defaultValue: 'List a New Property' })}</h1>
                    <p>Fill out the details below to list your property on HOMi.</p>
                  </div>
                  <div className="header-actions">
                    ...
                    <button className="close-page-btn" onClick={() => navigate('/my-properties')} title="Go back to My Properties"><FaTimes /></button>
                  </div>
                </div>

                {/* Gorgeous Horizontal Stepper */}
                <div className="horizontal-stepper">
                  <button type="button" onClick={() => handleTabClick(1)} className={`step-node ${step >= 1 ? 'active' : ''} ${step > 1 ? 'completed' : ''}`}>
                    <span className="step-num">{step > 1 ? '✓' : '1'}</span>
                    <span className="step-txt">{t('myProperties.tabs.general')}</span>
                  </button>
                  <div className={`step-line ${step > 1 ? 'completed' : ''}`}></div>
                  <button type="button" onClick={() => handleTabClick(2)} className={`step-node ${step >= 2 ? 'active' : ''} ${step > 2 ? 'completed' : ''}`}>
                    <span className="step-num">{step > 2 ? '✓' : '2'}</span>
                    <span className="step-txt">{t('myProperties.tabs.media')}</span>
                  </button>
                  <div className={`step-line ${step > 2 ? 'completed' : ''}`}></div>
                  <button type="button" onClick={() => handleTabClick(3)} className={`step-node ${step >= 3 ? 'active' : ''} ${step > 3 ? 'completed' : ''}`}>
                  ...
                  <button type="button" onClick={() => handleTabClick(4)} className={`step-node ${step >= 4 ? 'active' : ''} ${step > 4 ? 'completed' : ''}`}>
                ...
                <div className="page-main-content">
                  ...
                  {submitError && (
                    <div className="property-page-error-text">
                      {submitError}
                    </div>
                  )}
                  ...
                  <div className="property-page-footer">
                    {step > 1 && (
